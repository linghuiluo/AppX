...
public class Converter{
    ...
    public Result convert(final QueryResponse ddbResponse,
                                      final String idAttributeName) {
        final List<Map<String, AttributeValue>> items = ddbResponse.hasItems()
            ? ddbResponse.items() : ImmutableList.of();
        final List<String> ids = items.stream()
            .map(attributeValueMap -> attributeValueMap.get(idAttributeName).s())

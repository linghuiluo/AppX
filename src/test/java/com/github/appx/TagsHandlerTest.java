...
public class TagsHandlerTest {
	...
	public void getTags_invalidInput_null() {
		...
		ApiGatewayResponse expected = ApiGatewayResponse.builder().setStatusCode(BAD_REQUEST.getStatus())
				.setObjectBody(classUnderTest.getErrMsg("invalid input - brand_id MUST be a number")).build();
		assertResponse(actual, expected);
	}
	...
	public void getTags_invalidInput_noLang() {
		...
		ApiGatewayResponse actual = classUnderTest.handleRequest(input, null);
		ApiGatewayResponse expected = ApiGatewayResponse.builder().setStatusCode(BAD_REQUEST.getStatus())
				.setObjectBody(classUnderTest.getErrMsg("internal server error - failed to parse input")).build();
		assertResponse(actual, expected);
	}
	...
	public void getTags_noTags() {
		...
		ApiGatewayResponse actual = classUnderTest.handleRequest(input, null);
		ApiGatewayResponse expected = ApiGatewayResponse.builder().setStatusCode(SERVER_ERROR.getStatus())
				.setObjectBody(classUnderTest.getErrMsg("failed to get TAGS from DB")).build();
		assertResponse(actual, expected);
	}
	...
	public void getTags_returnNullTags() {
		...
		ApiGatewayResponse actual = classUnderTest.handleRequest(input, null);
		ApiGatewayResponse expected = ApiGatewayResponse.builder().setStatusCode(SERVER_ERROR.getStatus())
				.setObjectBody(classUnderTest.getErrMsg("failed to retrieve tags content")).build();
		assertResponse(actual, expected);
	}
